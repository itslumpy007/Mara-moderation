...
export function createStaffTools({store,env,audit,backupNow,verification,tickets}) {
  ...
  return async (i,actor)=>{
    ...
    let result;
    if(n==='config') { await save({[s('setting')]:s('id')==='-'?'':s('id')}); result='Saved. This setting takes effect immediately and overrides the Railway value. Run /setup-check to verify it.'; }
    else if(n==='welcome') {
