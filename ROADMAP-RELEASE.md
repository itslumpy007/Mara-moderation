...
### Rules-first verification
The Verify button now opens a private rules card. Members must accept the current rules before proceeding. With CAPTCHA enabled, the next button opens the verification website: sign in with Discord, view/accept the same rules, then complete Turnstile before receiving a role. Discord cannot embed this web CAPTCHA in a message. Existing panels use the new flow, but repost /panel kind:verify to update their button label to Verify. Generic rules ship by default; customize using /verification rules:<text> (up to 1500 characters). Rules edits invalidate old acceptance buttons and stale web submissions. CAPTCHA requires the existing dashboard, OAuth and Turnstile settings, then /verification captcha:true. Without CAPTCHA enabled, accepting the rules grants the role after the existing checks.

### Unverified role on join
Set UNVERIFIED_ROLE_ID in Railway or use /config setting:UNVERIFIED_ROLE_ID id:<role ID>. Mara gives new human members this role, then removes it after successful button, CAPTCHA, or staff verification. Existing members are not backfilled. Keep both roles below Mara, give Mara Manage Roles, and use separate non-privileged roles. Configure channel permissions in Discord to restrict Unverified members as desired. Assignment failures are logged. If removal fails after the verified role is granted, retry verification to clean up the old role.
